...
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
// IMPORTAR TRANSFERÊNCIA PORTÁTIL
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
async function importarTransferencia(input) {
  const file = input.files[0];
  if (!file) return;

  const status = document.getElementById('import-transfer-status');
  ...
  try {
    ...
    if (data.type !== 'nagumo-transfer' || data.version !== '3.0') {
      if (status) status.textContent = '❌ Arquivo não é uma transferência válida.';
      showToast('❌ Formato inválido.','error');
      input.value = '';
      return;
    }
    if (!data.employee || !data.employee.id) {
      if (status) status.textContent = '❌ Arquivo corrompido: funcionário não encontrado.';
      showToast('❌ Arquivo inválido.','error');
      input.value = '';
      return;
    }
    ...
    const existingIdx = store.employees.findIndex(e => String(e.id) === String(empId));

    if (existingIdx >= 0) {
      if (status) status.textContent = '⚠️ Funcionário já existe neste sistema. Use outro ID.';
      showToast('⚠️ Funcionário já existe.','warn');
      input.value = '';
      return;
    }
    ...
    data.employee.id = maxId + 1;

    // Adiciona ao store
    store.employees.push(data.employee);
    await saveStore(store);

    // Salva ponto mensal
    if (data.ponto) {
      for (const [mk, pontoData] of Object.entries(data.ponto)) {
        const existing = await window.DB.loadData('ponto', mk);
        if (!existing) await window.DB.loadData('ponto', mk);
        const pontoStore = await window.DB.loadData('ponto', mk) || { registros: [] };
        if (!pontoStore.registros) pontoStore.registros = [];
        pontoStore.registros.push(pontoData);
        await window.DB.saveData('ponto', mk, pontoStore);
      }
    }

    // Salva ocorrências
    if (data.ocorrencias?.length) {
      const allOcorr = await LS.get('rh_ocorrencias', []);
      const newOcorr = data.ocorrencias.map(o => ({ ...o, emp_id: data.employee.id, id: undefined }));
      allOcorr.push(...newOcorr);
      await LS.set('rh_ocorrencias', allOcorr);
    }

    // Salva aptidões
    if (data.aptidoes) {
      for (const [mk, aptData] of Object.entries(data.aptidoes)) {
        const aptStore = await window.DB.loadData('aptidoes', mk) || { avaliacoes: [] };
        if (!aptStore.avaliacoes) aptStore.avaliacoes = [];
        aptData.emp_id = data.employee.id;
        aptStore.avaliacoes.push(aptData);
        await window.DB.saveData('aptidoes', mk, aptStore);
      }
    }
    ...
    if (status) status.textContent = `✅ Funcionário ${data.employee.name} importado com sucesso!`;
    await registrarLog('importou', 'Funcionários', `Importou transferência: <strong>${data.employee.name}</strong>`);
    showToast(`✅ ${data.employee.name} importado!`);
    input.value = '';
  } catch (e) {
    console.error('[importarTransferencia]', e);
    if (status) status.textContent = '❌ Erro ao processar arquivo.';
...
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
// EXPORTAR / IMPORTAR MENSAL
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
async function exportarMes() {
  const mk = document.getElementById('monthly-export-month')?.value;
  if (!mk || !/^\d{4}-\d{2}$/.test(mk)) {
    showToast('⚠️ Selecione um mês válido.','warn');
    return;
  }

  const store = await getStore();
  const myLeaders = currentUser?.nivel === 'admin_master'
    ? Object.values(store.users || {}).filter(u => u.nivel !== 'admin_master')
    : [currentUser];

  const lideresIds = myLeaders.map(l => String(l.id));
  ...
  const dados = {
    ...
    month: mk,
    exported_at: new Date().toISOString(),
    exported_by: currentUser?.email || currentUser?.id || 'unknown',
    ponte: {},
    ocorrencias: [],
    aptidoes: {},
  };
  ...
  // Ponto do mês
  const pontoData = await window.DB.loadData('ponto', mk);
  if (pontoData?.registros) {
    dados.ponte = pontoData;
  }

  // Ocorrências do mês
  const allOcorr = await LS.get('rh_ocorrencias', []);
  const startDate = new Date(mk + '-01T00:00:00');
  const endDate = new Date(startDate);
  endDate.setMonth(endDate.getMonth() + 1);
  dados.ocorrencias = allOcorr.filter(o => {
    if (!lideresIds.includes(String(o.lider_id))) return false;
    const oDate = new Date(o.data || o.created_at);
    return oDate >= startDate && oDate < endDate;
  });

  // Aptidões do mês
  const aptData = await window.DB.loadData('aptidoes', mk);
  if (aptData?.avaliacoes) {
    dados.aptidoes = aptData;
  }
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `nagumo-${mk}-${currentUser?.name || 'lider'}.json`;
  a.click();
  URL.revokeObjectURL(url);
  await registrarLog('exportou', 'Configurações', `Exportou mês <strong>${mk}</strong>`);
  showToast(`✅ Mês ${mk} exportado!`);
}

async function importarMes(input) {
  const file = input.files[0];
  if (!file) return;

  const status = document.getElementById('monthly-import-status');
  if (status) status.textContent = '⏳ Lendo arquivo...';
  ...
  try {
    ...
    if (data.type !== 'nagumo-monthly' || data.version !== '3.0') {
      if (status) status.textContent = '❌ Arquivo não é uma exportação mensal válida.';
      showToast('❌ Formato inválido.','error');
      input.value = '';
      return;
    }
    if (!data.month || !/^\d{4}-\d{2}$/.test(data.month)) {
      if (status) status.textContent = '❌ Mês inválido no arquivo.';
    ...
    // Importa ponto
    if (data.ponte?.registros?.length) {
      const existing = await window.DB.loadData('ponto', mk) || { registros: [] };
      if (!existing.registros) existing.registros = [];
      for (const reg of data.ponte.registros) {
        const existsIdx = existing.registros.findIndex(r => String(r.id) === String(reg.id));
        if (existsIdx >= 0) {
          existing.registros[existsIdx] = reg;
        } else {
          existing.registros.push(reg);
        }
      }
      await window.DB.saveData('ponto', mk, existing);
    }

    // Importa ocorrências
    if (data.ocorrencias?.length) {
      const allOcorr = await LS.get('rh_ocorrencias', []);
      for (const ocorr of data.ocorrencias) {
        const existsIdx = allOcorr.findIndex(o => String(o.id) === String(ocorr.id));
        if (existsIdx >= 0) {
          allOcorr[existsIdx] = ocorr;
        } else {
          allOcorr.push(ocorr);
        }
      }
    ...
    // Importa aptidões
    if (data.aptidoes?.avaliacoes?.length) {
      const existing = await window.DB.loadData('aptidoes', mk) || { avaliacoes: [] };
      if (!existing.avaliacoes) existing.avaliacoes = [];
      for (const apt of data.aptidoes.avaliacoes) {
        const existsIdx = existing.avaliacoes.findIndex(a => String(a.emp_id) === String(apt.emp_id));
        if (existsIdx >= 0) {
          existing.avaliacoes[existsIdx] = apt;
        } else {
          existing.avaliacoes.push(apt);
        }
      }
      await window.DB.saveData('aptidoes', mk, existing);
    }

    _store = null;
    _allPonto = null;

    if (status) status.textContent = `✅ Mês ${mk} importado com sucesso!`;
    await registrarLog('importou', 'Configurações', `Importou mês <strong>${mk}</strong>`);
    showToast(`✅ Mês ${mk} importado!`);
    input.value = '';
  } catch (e) {
    console.error('[importarMes]', e);
    if (status) status.textContent = '❌ Erro ao processar arquivo.';
    showToast('❌ Erro ao importar.','error');
    input.value = '';
  }
...
// MIGRAR DO SQLITE
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
async function migrarDoSQLite() {
  if (currentUser?.nivel !== 'admin_master') {
    showToast('⚠️ Apenas o Admin Master pode migrar dados.','warn');
    return;
  }

  const status = document.getElementById('migrate-status');
  if (status) status.textContent = '⏳ Iniciando migração...';

  const btn = document.getElementById('btn-migrate');
  if (btn) btn.disabled = true;

  const ok = await window.DB.migrateFromSQLite(status);

  if (btn) btn.disabled = false;

  if (ok) {
    await registrarLog('migrou', 'Configurações', 'Migração do SQLite concluída');
    showToast('✅ Migração concluída! Reinicie o app.');
  }
}
