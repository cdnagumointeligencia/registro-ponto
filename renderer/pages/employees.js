...
function buildCard(emp) {
  ...
  <div class="emp-card ${emp.maternidade ? 'maternidade-card' : ''}" style="${emp.afastado?'border-color:rgba(239,68,68,.4)':emFerias?'border-color:rgba(34,211,238,.4)':''}">
    ...
    <div class="emp-actions-bottom">
      ...
      ${podTransfer ? `<button class="emp-btn export" data-action="export-transfer" data-id="${emp.id}" title="Exportar para Transferência">📦</button>` : ''}
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('emp-grid')?.addEventListener('click', e => {
    ...
    else if (act === 'export-transfer') openExportTransferModal(id);
...
async function saveEmployee() {
  ...
  if (editId) {
    ...
    else dataFields.pass = await hashPassword(pass);
    ...
    const novoEmp = { id: newId, ...dataFields, historico_cargos, supervisor_id, pass: pass ? await hashPassword(pass) : null };
...
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
// EXPORTAR TRANSFERÊNCIA PORTÁTIL
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
function openExportTransferModal(id) {
  const emp = (_store?.employees||[]).find(e=>String(e.id)===String(id));
  if (!emp) return;
  document.getElementById('exp-emp-name').textContent = emp.name;
  document.getElementById('exp-edit-id').value = id;
  document.getElementById('modal-export-transfer').classList.add('open');
}

function closeExportTransferModal() {
  document.getElementById('modal-export-transfer').classList.remove('open');
}

async function confirmarExportTransfer() {
  const id = document.getElementById('exp-edit-id').value;
  const store = await getStore();
  const emp = (store.employees||[]).find(e=>String(e.id)===String(id));
  if (!emp) { showToast('❌ Funcionário não encontrado.','error'); return; }

  const dados = {
    type: 'nagumo-transfer',
    version: '3.0',
    employee: emp,
    ponto: {},
    ocorrencias: [],
    aptidoes: {},
    competencies_history: emp.competencies_history || {},
    perf_history: emp.perf_history || {},
    absences: emp.absences || [],
    history: emp.history || [],
    exported_at: new Date().toISOString(),
    exported_by: currentUser?.email || currentUser?.id || 'unknown',
    exported_by_name: currentUser?.name || 'unknown',
  };

  // Coleta dados mensais de ponto
  const months = await window.DB.listMonths ? window.DB.listMonths() : [];
  for (const mk of months) {
    const pontoData = await window.DB.loadData('ponto', mk);
    if (pontoData?.registros) {
      const empPonto = pontoData.registros.find(r => String(r.id) === String(id));
      if (empPonto) dados.ponto[mk] = empPonto;
    }
  }

  // Coleta ocorrências
  const allOcorr = await LS.get('rh_ocorrencias', []);
  dados.ocorrencias = allOcorr.filter(o => String(o.emp_id) === String(id) || String(o.ref_id) === String(id));

  // Coleta aptidões
  for (const mk of months) {
    const aptData = await window.DB.loadData('aptidoes', mk);
    if (aptData?.avaliacoes) {
      const empApt = aptData.avaliacoes.find(a => String(a.emp_id) === String(id) || String(a.id) === String(id));
      if (empApt) dados.aptidoes[mk] = empApt;
    }
  }
  ...
  a.href = url;
  a.download = `transferencia-${safeName}-${date}.json`;
  a.click();
  URL.revokeObjectURL(url);

  closeExportTransferModal();
  await registrarLog('exportou', 'Funcionários', `Exportou transferência: <strong>${emp.name}</strong>`);
  showToast('✅ Arquivo de transferência gerado!');
}

document.getElementById('modal-export-transfer')?.addEventListener('click', e => {
  if (e.target === e.currentTarget) closeExportTransferModal();
});
document.getElementById('modal-export-transfer')?.addEventListener('keydown', e => {
  if (e.key === 'Escape') closeExportTransferModal();
});

// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
// OCORRÊNCIA DO FUNCIONÁRIO
// """"""""""""""""""""""""""""""""""""""""""""""""""""""""
