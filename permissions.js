...
function canEdit(user, page, employee) {
  ...
  // Escopo próprio: funcionário precisa ter sido cadastrado por este usuário
  if (scope.own) {
    return (employee?.owner_id || employee?.ownerId) === user.id;
  }
...
// Salvos em localStorage separado para não misturar com dados
// ════════════════════════════════════════════════════════
...
function savePermissionOverrides(overrides) {
  try { localStorage.setItem(PERM_OVERRIDE_KEY, JSON.stringify(overrides)); } catch(e) {}
}
